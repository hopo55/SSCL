...
class cifar_dataset(Dataset): 
    def __init__(self, dataset, X_images, X_labels, U_images, root_dir, transform, mode, noise_file='', pred=[], probability=[], log=''): 
        ...
        if self.mode=='test':
            ...
            train_data_labelled = X_images
            train_label_labelled = X_labels
            train_data_unlabelled = U_images
...
class cifar_dataloader():  
    ...
    def run(self,sample_ratio, X_images, X_labels, U_images, mode, class_name=[], pred=[], prob=[]):
                
        if mode=='train':
            labeled_dataset = cifar_dataset(dataset=self.dataset, X_images = X_images,  X_labels = X_labels,  U_images = U_images, root_dir=self.root_dir, transform=self.transforms["labeled"], mode="labeled", noise_file=self.noise_file, pred=pred, probability=prob,log=self.log)              
            ...
            unlabeled_dataset = cifar_dataset(dataset=self.dataset, X_images = X_images,  X_labels = X_labels,  U_images = U_images, root_dir=self.root_dir, transform=self.transforms["unlabeled"], mode="unlabeled", noise_file=self.noise_file, pred=pred)                    
            ...
            test_dataset = cifar_dataset(dataset=self.dataset, X_images = [],  X_labels =[],  U_images = [], root_dir=self.root_dir, transform=self.transform_test, mode='test')      
