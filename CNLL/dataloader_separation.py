...
import wandb
wandb.init(project="noisy-label-project", entity="hopo55")
...
class cifar_dataset(Dataset): 
    def __init__(self, dataset, task_mode, noise_mode, ratio, iteration, delay_size, root_dir, transform, mode, noise_file='', pred=[], probability=[], log=''): 
        ...
        if self.mode=='test':
            ...
            ## Shuffle the data
            random_idx = np.random.permutation(np.arange(num_sample))
            train_data  = train_data[random_idx]
            noise_label = train_label[random_idx]

            for kk in range(num_class):
                self.class_ind[kk] = [i for i,x in enumerate(noise_label) if x==kk]

            if self.mode == 'all':
                self.train_data  = train_data[iteration[0]*delay_size:iteration[1]*delay_size]
                self.noise_label = noise_label[iteration[0]*delay_size:iteration[1]*delay_size]
...
class cifar_dataloader():  
    ...
    def run(self, SR, iteration, delay_buffer_size, mode, class_name=[], pred=[], prob=[]):
        if mode=='warmup':
            all_dataset = cifar_dataset(dataset=self.dataset, task_mode= self.task_mode, noise_mode = self.noise_mode, ratio = self.r, iteration= iteration, delay_size = delay_buffer_size, root_dir=self.root_dir, transform=self.transforms["warmup"], mode="all",noise_file=self.noise_file)                
            ...
            labeled_dataset = cifar_dataset(dataset=self.dataset, task_mode= self.task_mode,  noise_mode = self.noise_mode, ratio = self.r, iteration= iteration, delay_size = delay_buffer_size, root_dir=self.root_dir, transform=self.transforms["labeled"], mode="labeled", noise_file=self.noise_file, pred=pred, probability=prob,log=self.log)              
            ...
            test_dataset = cifar_dataset(dataset=self.dataset, task_mode= self.task_mode,  noise_mode = self.noise_mode, ratio = self.r, iteration= iteration, delay_size = delay_buffer_size, root_dir=self.root_dir, transform=self.transform_test, mode='test')      
            ...
            eval_dataset = cifar_dataset(dataset=self.dataset, task_mode= self.task_mode, noise_mode = self.noise_mode, ratio = self.r, iteration= iteration, delay_size = delay_buffer_size, root_dir=self.root_dir, transform=self.transform_test, mode='all', noise_file=self.noise_file)      
