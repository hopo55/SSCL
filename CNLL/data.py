...
class DataScheduler():
    def __init__(self, config):
        ...
        for stage in self.schedule:
            ...
            # in below iter code works okay, but if you have experts trained
    ...
    def __iter__(self):
        for t_i, task in enumerate(self.task_datasets):
            ...
            for data in DataLoader(task, batch_size=self.config['batch_size'], num_workers=self.config['num_workers'],
                                   collate_fn=collate_fn, drop_last=False, shuffle=False): # shuffle=True
                yield data, t_i
        ...
        for subset_name, subset in self.subsets.items():
            if subset_name in lab_uni:
                ...
                for x, y in iter(data):
                    ...
                    # remapped_targets = torch.zeros(lab.size(), dtype= torch.long )
                    # for t in range(lab.size()[0]):
                    #     remapped_targets[t] = (lab[t] == lab_uni[1])

                    test_image_features.extend(list(images.data.cpu().numpy()))                
                    test_labels.extend(lab.data.cpu())
                    total += batch_size
